...
interaction_images = {
    'e01':os.path.join('/home/eric/Documents','images/move_forward_original.png'),
    'e00':os.path.join('/home/eric/Documents','images/bump_original.png'),
    'e11':os.path.join('/home/eric/Documents','images/turn_left.png'),
    'e21':os.path.join('/home/eric/Documents','images/turn_right.png'),
    'e31':os.path.join('/home/eric/Documents','images/feel_front_empty.png'),
    'e30':os.path.join('/home/eric/Documents','images/feel_front_wall.png'),
    'e41':os.path.join('/home/eric/Documents','images/feel_left_empty.png'),
    'e40':os.path.join('/home/eric/Documents','images/feel_left_wall.png'),
    'e51':os.path.join('/home/eric/Documents','images/feel_right_empty.png'),
    'e50':os.path.join('/home/eric/Documents','images/feel_right_wall.png')
}
...
def addOrGetAndReinforceCompositeInteraction(previousInteraction,lastInteraction,isFlag):
    ...
    if label in INTERACTIONS:#the interaction already in the INTERACTIONS
        if isFlag:
...
def enactionClient(request_string):
    rospy.wait_for_service('enaction')
    try:
        enactionService = rospy.ServiceProxy('enaction', EnactionSrv)
        enactedResponse = enactionService(request_string)
        return enactedResponse.enactedInteraction
    except rospy.ServiceException as e:
        print("Service call failed: %s"%e)

def getIntendedInteractionFormat(intendedInteractionLabel):
    interactionsStringWithResponseFormat = ""
    eIndex = 0
    eIndexList = []
    interactionList = []
    for i in intendedInteractionLabel:
        if i == 'e':
            eIndexList.append(eIndex)
        eIndex+=1
    for ei in eIndexList:
        interaction = intendedInteractionLabel[ei:ei+3]
        interactionList.append(interaction)
    for interaction in interactionList:
        interactionsStringWithResponseFormat = interactionsStringWithResponseFormat + interaction+","
    return interactionsStringWithResponseFormat
...
def interaction(canvas,count):
    ...
    if startState:
        ...
        while loopNum < 200:
            ...
            intended_response = getIntendedInteractionFormat(intendedInteraction.label)
            print("++ intended_response is: "+intended_response)

            #enacted_response = enactionClient(intendedInteraction.strInteraction())
            enacted_response = enactionClient(intendedInteraction.label)
            print("++ enacted_response  is: "+enacted_response)

            currentEnactedInteraction = intendedInteraction
            if intended_response == enacted_response:
                currentEnactedInteraction = intendedInteraction
            else:#intendedInteraction != enactedInteraction
                if intendedInteraction.isPrimitive():
                    currentEnactedInteraction = INTERACTIONS[enacted_response[0:3]]
                else:
                    nodeList = []
                    nodeStack = []
                    upperNode = Node(intendedInteraction)
                    nodeStack.append(upperNode)
                    nodeList.append(upperNode)
                    while nodeStack:
                        upperNode = nodeStack.pop()
                        if not upperNode.interaction.isPrimitive():
                            rightNode = Node(upperNode.interaction.postInteraction)
                            rightNode.upperNode = upperNode
                            rightNode.upperDirection = "right"
                            upperNode.rightNode = rightNode
                            nodeStack.append(rightNode)
                            nodeList.append(rightNode)

                            leftNode = Node(upperNode.interaction.preInteraction)
                            leftNode.upperNode = upperNode
                            leftNode.upperDirection = "left"
                            upperNode.leftNode = leftNode
                            nodeStack.append(leftNode)
                            nodeList.append(rightNode)

                    farLeftNode = getFarLeft(nodeList)
                    #e31,e31,e41,
                    currentNode = farLeftNode
                    enactedNodeStack = []
                    enactedResponseInteractions = enacted_response.split(',')
                    for enactedResponseInteraction in enactedResponseInteractions:
                        if enactedResponseInteraction != '':
                            if enactedResponseInteraction == currentNode.interaction.label:
                                if currentNode.upperDirection == "left":
                                    enactedNodeStack.append(currentNode)
                                else:#the right side
                                    currentStackNode = currentNode
                                    while currentStackNode.upperDirection == "right":
                                        currentStackNode = currentStackNode.upperNode
                                        enactedNodeStack.pop()
                                        if currentStackNode.upperDirection == "left":
                                            enactedNodeStack.append(currentStackNode.upperNode)
                            else:
                                enactedPrimitive = INTERACTIONS[enactedResponseInteraction]
                                temporaryInteraction = enactedPrimitive
                                while enactedNodeStack:
                                    popInteraction = enactedNodeStack.pop()
                                    temporaryInteraction = addOrGetAndReinforceCompositeInteraction(popInteraction.interaction,temporaryInteraction,True)
                                currentEnactedInteraction = temporaryInteraction
...
def getFarLeft(nodeList):
    farLeftNode = nodeList[0]
    while farLeftNode.leftNode is not None:
        farLeftNode = farLeftNode.leftNode
    return farLeftNode
